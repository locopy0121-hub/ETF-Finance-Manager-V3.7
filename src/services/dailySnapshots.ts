...
export function buildDailySnapshot(holdings:Holding[],quotes:Record<string,TwseQuote>,ledger:LedgerEntry[]=[],dividends:DividendEvent[]=[],at=new Date(),cashBalance=0):DailySnapshot{
 ...
 return {
  id:`snapshot-${dayKey(at)}`,date:dayKey(at),createdAt:at.getTime(),marketValue:p.marketValue,cashBalance:p.cashBalance,totalAssets:p.totalAssets,totalCost:p.currentCashBasis,
  ...
  todayPnl:p.todayPnl,todayPnlPct:p.todayPnlPct/100,costPnl:p.cashUnrealizedPnl,costPnlPct,totalPnl:p.totalPnl,totalPnlPct,cumulativeDividend:p.cumulativeDividends,holdings:rows,
 };
}
