...
export function floatingOverlayPayload(args:{prefs:V3Preferences;holdings:Holding[];quotes:Record<string,any>;ledger:LedgerEntry[];dividends:DividendEvent[];cashBalance:number;lastSuccessAt?:number;marketState:string;monitorInstance?:MonitorInstanceState}){
 ...
 const fields:MonitorField[]=[...(profile?.fieldsCustomized===true&&profile?.fields?.length?profile.fields:modeConfig.displayFields as MonitorField[])];
 ...
 return JSON.stringify({
  ...
  normalLayout,miniLayout,isMinimized,activeTemplateId:template.id,modeConfig,
  ...
  width:activeLayout.width,height:activeLayout.height,minWidth:profile?.minWidth??120,minHeight:profile?.minHeight??48,maxHeightRatio:profile?.maxHeightRatio??.72,autoHeight:template.compact===true,
  ...
  symbolStyle:'inventory',background:modeConfig.backgroundColor,textColor:prefs.primaryTextColor??'#FFFFFF',
