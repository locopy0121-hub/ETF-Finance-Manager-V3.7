...
export type MonitorProfile={
 ...
 selectedSymbols:string[]; fields:MonitorField[]; maxSymbols:number; sortMode:MonitorSortMode; groupTabs:boolean;
...
export const makeMonitorProfile=(kind:'app'|'floating'|'widget'):MonitorProfile=>({
 enabled:kind!=='app'?false:true,customizeMode:false,displayMode:kind==='floating'?'holdingList':'cardMatrix',resizeMode:'fluid',symbolSource:'holdings',title:'即時監控器',statusTitle:'市場狀態',showBreathingLight:true,density:'auto',selectedSymbols:[],fields:[...defaultMonitorFields],maxSymbols:10,sortMode:'custom',groupTabs:true,
...
export function mergeMonitorProfile(base:MonitorProfile,raw:any):MonitorProfile{const legacyMode:Record<string,MonitorDisplayMode>={smart:'holdingList',list:'holdingList',puzzle:'cardMatrix'};const merged={...base,...(raw??{}),displayMode:legacyMode[raw?.displayMode]??raw?.displayMode??base.displayMode,selectedSymbols:Array.isArray(raw?.selectedSymbols)?raw.selectedSymbols:base.selectedSymbols,fields:Array.isArray(raw?.fields)?raw.fields:base.fields,puzzleTiles:Array.isArray(raw?.puzzleTiles)?raw.puzzleTiles:base.puzzleTiles,fieldStyles:{...base.fieldStyles,...(raw?.fieldStyles??{})},schedule:{...base.schedule,...(raw?.schedule??{})}};if(merged.tapAction==='openApp')merged.tapAction='none';return merged;}
