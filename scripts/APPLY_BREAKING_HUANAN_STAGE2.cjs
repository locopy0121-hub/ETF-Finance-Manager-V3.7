...
rx('App.tsx',/ const addCash=\(x:\{amount:number;date:string;broker:string;brokerProfileId:string;account:string;note\?:string\}\)=>patch\(s=>\(\{[\s\S]*?\}\}\)\)\);/,` const addCash=(x:{amount:number;date:string;account:string;note?:string})=>patch(s=>({...s,cashBalance:s.cashBalance+x.amount,ledger:[...s.ledger,{id:id(),kind:x.amount>=0?'cashIn':'cashOut',date:x.date,amount:Math.abs(x.amount),broker:'華南永昌證券',account:x.account||undefined,note:x.note}]}));`, 'addCash');
